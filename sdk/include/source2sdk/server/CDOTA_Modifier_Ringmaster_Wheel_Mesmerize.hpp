...
namespace source2sdk::server
{
    ...
    public:
        ...
        float wheel_stun; // 0x16f0        
        float mesmerize_radius; // 0x16f4        
        float vision_cone; // 0x16f8        
        float explosion_damage; // 0x16fc        
        server::CountdownTimer m_ctFuseTimer; // 0x1700        
        client::ParticleIndex_t m_nFXIndex; // 0x1718        
        int32_t m_nTimesTriggered; // 0x171c        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Ringmaster_Wheel_Mesmerize because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Ringmaster_Wheel_Mesmerize) == 0x1720);
};
