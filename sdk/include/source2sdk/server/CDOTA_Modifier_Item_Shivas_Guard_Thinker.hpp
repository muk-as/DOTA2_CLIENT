...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            std::int32_t blast_speed; // 0x18c0            
            float blast_radius; // 0x18c4            
            std::int32_t blast_damage; // 0x18c8            
            std::int32_t illusion_multiplier_pct; // 0x18cc            
            float blast_debuff_duration; // 0x18d0            
            float resist_debuff_duration; // 0x18d4            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Item_Shivas_Guard_Thinker because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Item_Shivas_Guard_Thinker) == 0x18d8);
    };
};
