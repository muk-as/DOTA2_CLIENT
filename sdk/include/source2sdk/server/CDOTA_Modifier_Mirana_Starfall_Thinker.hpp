...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float starfall_secondary_radius; // 0x_            
            float starstruck_duration; // 0x_            
            // m_hTarget has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::server::CBaseEntity> m_hTarget;
            char m_hTarget[0x_]; // 0x_            
            float m_flDamage; // 0x_            
            bool m_bStarDropped; // 0x_            
            bool m_bSecondStar; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Mirana_Starfall_Thinker) == 0x_);
    };
};
