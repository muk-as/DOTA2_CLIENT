...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float damage_per_tree_pct; // 0x_            
            float radius; // 0x_            
            float multiplier; // 0x_            
            float radius_treant; // 0x_            
            std::int32_t m_nTreeAmount; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Furion_SpiritOfTheForest) == 0x_);
    };
};
