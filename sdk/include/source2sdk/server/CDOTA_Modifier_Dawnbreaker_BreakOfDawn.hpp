...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float max_dmg_pct; // 0x_            
            float max_vision_pct; // 0x_            
            float m_flDaytimeStart; // 0x_            
            float m_flNighttimeStart; // 0x_            
            bool m_bDaytimeNormal; // 0x_            
            uint8_t _pad_[0x_]; // 0x_
            float day_threshold; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Dawnbreaker_BreakOfDawn because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Dawnbreaker_BreakOfDawn) == 0x_);
    };
};
