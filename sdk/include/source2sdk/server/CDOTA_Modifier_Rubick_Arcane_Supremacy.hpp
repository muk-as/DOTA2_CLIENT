...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t cast_range; // 0x1878            
            std::int32_t spell_amp; // 0x187c            
            float aoe_bonus; // 0x1880            
            float aoe_bonus_duration; // 0x1884            
            ...
            char m_vecAoEExpireTimes[0x18]; // 0x1888            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Rubick_Arcane_Supremacy because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Rubick_Arcane_Supremacy) == 0x18a0);
    };
};
