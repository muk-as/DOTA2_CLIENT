...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            std::int32_t spill_speed; // 0x_            
            std::int32_t spill_radius; // 0x_            
            std::int32_t spill_damage; // 0x_            
            float spill_debuff_duration; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Item_Slime_Vial_Thinker because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Item_Slime_Vial_Thinker) == 0x_);
    };
};
