...
namespace source2sdk::server
{
    ...
    public:
        ...
        int32_t m_nMangoSeconds; // 0x16ec        
        server::CountdownTimer m_MangoTimer; // 0x16f0        
        int32_t m_nRespawnSeconds; // 0x1708        
        [[maybe_unused]] std::uint8_t pad_0x170c[0x4]; // 0x170c
        server::CountdownTimer m_RespawnTimer; // 0x1710        
        int32_t m_nMangosAvailable; // 0x1728        
        int32_t m_nChannelCount; // 0x172c        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Passive_Mango_Tree because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Passive_Mango_Tree) == 0x1730);
};
