...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float bonus_magic_resistance; // 0x_            
            float bonus_movespeed; // 0x_            
            float size_decrease; // 0x_            
            float armor_decrease; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_MetamorphicMandible_Active because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_MetamorphicMandible_Active) == 0x_);
    };
};
