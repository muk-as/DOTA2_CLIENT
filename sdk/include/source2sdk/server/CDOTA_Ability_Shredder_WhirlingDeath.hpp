...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            float whirling_radius; // 0x_            
            std::int32_t whirling_damage; // 0x_            
            float whirling_tick; // 0x_            
            float duration; // 0x_            
            std::int32_t tree_damage_scale; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::server::CDOTA_Ability_Shredder_WhirlingDeath) == 0x_);
    };
};
