...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_hDamaged[0x18]; // 0x1888            
            // m_hTarget has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::server::CBaseEntity> m_hTarget;
            char m_hTarget[0x4]; // 0x18a0            
            Vector m_vStartPosition; // 0x18a4            
            uint8_t _pad18b0[0x14]; // 0x18b0
            std::int32_t speed; // 0x18c4            
            float stun_radius; // 0x18c8            
            float radius_ally; // 0x18cc            
            float cooldown_refund_ally; // 0x18d0            
            float damage; // 0x18d4            
            float duration; // 0x18d8            
            float ally_shield_duration; // 0x18dc            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Rattletrap_Hookshot because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Rattletrap_Hookshot) == 0x18e0);
    };
};
