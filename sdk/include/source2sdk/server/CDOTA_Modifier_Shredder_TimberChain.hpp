...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            std::int32_t speed; // 0x1820            
            std::int32_t radius; // 0x1824            
            std::int32_t damage; // 0x1828            
            std::int32_t tree_splinter_count; // 0x182c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Shredder_TimberChain because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Shredder_TimberChain) == 0x1830);
    };
};
