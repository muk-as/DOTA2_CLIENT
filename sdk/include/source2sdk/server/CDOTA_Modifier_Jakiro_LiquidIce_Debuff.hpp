...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            float pct_health_damage; // 0x17fc            
            std::int32_t movement_slow; // 0x1800            
            std::int32_t slow_attack_speed_pct; // 0x1804            
            float tick_rate; // 0x1808            
            std::int32_t bonus_instance_damage_from_other_abilities; // 0x180c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Jakiro_LiquidIce_Debuff because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Jakiro_LiquidIce_Debuff) == 0x1810);
    };
};
