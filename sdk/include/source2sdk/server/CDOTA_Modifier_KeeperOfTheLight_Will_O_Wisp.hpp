...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t fixed_movement_speed; // 0x1888            
            Vector m_vSource; // 0x188c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_KeeperOfTheLight_Will_O_Wisp because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_KeeperOfTheLight_Will_O_Wisp) == 0x1898);
    };
};
