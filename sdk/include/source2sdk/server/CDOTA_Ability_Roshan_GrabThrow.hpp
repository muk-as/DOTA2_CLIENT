...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            source2sdk::server::CDOTA_BaseNPC* m_hTarget; // 0x_            
            std::int32_t m_nComboCount; // 0x_            
            float max_knockback_distance; // 0x_            
            float knockback_duration; // 0x_            
            float knockback_height; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Ability_Roshan_GrabThrow because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Ability_Roshan_GrabThrow) == 0x_);
    };
};
