...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t NUM_SOUNDS; // 0x1888            
            float dance_interval; // 0x188c            
            float gesture_duration; // 0x1890            
            float catch_up_distance; // 0x1894            
            float slow_duration; // 0x1898            
            float slow_amount; // 0x189c            
            std::int32_t m_nGesture; // 0x18a0            
            ...
            char m_vecDancers[0x18]; // 0x18a8            
            uint8_t _pad18c0[0x30]; // 0x18c0
            bool m_bIsGesturing; // 0x18f0            
            uint8_t _pad18f1[0x7];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Seasonal_TI11_CongaLine because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Seasonal_TI11_CongaLine) == 0x18f8);
    };
};
