...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t cleave_damage_pct; // 0x_            
            float cleave_starting_width; // 0x_            
            std::int32_t cleave_angle; // 0x_            
            float cleave_distance_base; // 0x_            
            std::int32_t cleave_distance_per_stack; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_AbyssalUnderlord_AtrophyAura_DmgBuffCounter) == 0x_);
    };
};
