...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t lifesteal_aura; // 0x_            
            std::int32_t damage_aura; // 0x_            
            float armor_aura; // 0x_            
            float mana_regen_aura; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Item_Vladmir_Aura because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Item_Vladmir_Aura) == 0x_);
    };
};
