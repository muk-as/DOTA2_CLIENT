#pragma once

#include "source2sdk/source2gen/source2gen.hpp"
#include <cstddef>
#include <cstdint>
#include "source2sdk/client/CDOTA_Buff.hpp"
#include "source2sdk/entity2/GameTime_t.hpp"
namespace source2sdk
{
    namespace server
    {
        struct CBaseEntity;
    };
};

// /////////////////////////////////////////////////////////////
// Module: server
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk
{
    namespace server
    {
        // Registered alignment: unknown
        // Alignment: unknown
        // Standard-layout class: false
        // Size: 0x1850
        // Has VTable
        #pragma pack(push, 1)
        class CDOTA_Modifier_Wisp_Tether : public source2sdk::client::CDOTA_Buff
        {
        public:
            bool m_bInManaGained; // 0x17f8            
            uint8_t _pad17f9[0x3]; // 0x17f9
            // m_hTarget has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CHandle<source2sdk::server::CBaseEntity> m_hTarget;
            char m_hTarget[0x4]; // 0x17fc            
            // hStunnedEntities has a template type with potentially unknown template parameters. You can try uncommenting the field below.
            // CUtlVector<CHandle<source2sdk::server::CBaseEntity>> hStunnedEntities;
            char hStunnedEntities[0x18]; // 0x1800            
            float stun_duration; // 0x1818            
            std::int32_t movespeed; // 0x181c            
            std::int32_t self_bonus; // 0x1820            
            bool m_bIsInRange; // 0x1824            
            uint8_t _pad1825[0x3]; // 0x1825
            float radius; // 0x1828            
            float latch_distance; // 0x182c            
            std::int32_t damage_absorb; // 0x1830            
            float m_flHealthHealed; // 0x1834            
            float m_flManaHealed; // 0x1838            
            float tether_heal_amp; // 0x183c            
            float tether_mana_amp; // 0x1840            
            source2sdk::entity2::GameTime_t m_flHealMessageTime; // 0x1844            
            source2sdk::entity2::GameTime_t m_flManaMessageTime; // 0x1848            
            uint8_t _pad184c[0x4];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Wisp_Tether because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Wisp_Tether) == 0x1850);
    };
};
