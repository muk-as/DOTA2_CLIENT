...
namespace source2sdk::server
{
    ...
    public:
        ...
        int32_t drain_rate; // 0x170c        
        float drain_length; // 0x1710        
        int32_t attack_speed_factor; // 0x1714        
        int32_t drain_armor; // 0x1718        
        [[maybe_unused]] std::uint8_t pad_0x171c[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Razor_StaticLink_Buff because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Razor_StaticLink_Buff) == 0x1720);
};
