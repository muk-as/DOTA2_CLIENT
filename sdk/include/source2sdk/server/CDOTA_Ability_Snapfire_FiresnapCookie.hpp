...
namespace source2sdk::server
{
    ...
    public:
        ...
        [[maybe_unused]] std::uint8_t pad_0x5cc[0x8]; // 0x5cc
        int32_t projectile_speed; // 0x5d4        
        float pre_hop_duration; // 0x5d8        
        float cast_on_ally_gesture_time; // 0x5dc        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Ability_Snapfire_FiresnapCookie because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Ability_Snapfire_FiresnapCookie) == 0x5e0);
};
