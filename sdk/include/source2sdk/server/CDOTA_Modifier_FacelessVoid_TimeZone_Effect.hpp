...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_vecAbilities[0xb8]; // 0x1888            
            std::int32_t bonus_move_speed; // 0x1940            
            std::int32_t bonus_attack_speed; // 0x1944            
            std::int32_t bonus_cast_speed; // 0x1948            
            std::int32_t bonus_turn_speed; // 0x194c            
            std::int32_t cooldown_acceleration; // 0x1950            
            uint8_t _pad1954[0x4];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_FacelessVoid_TimeZone_Effect because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_FacelessVoid_TimeZone_Effect) == 0x1958);
    };
};
