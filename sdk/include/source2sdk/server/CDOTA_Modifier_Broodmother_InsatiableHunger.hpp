...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            std::int32_t lifesteal_pct; // 0x_            
            float slow_duration; // 0x_            
            float bat_bonus; // 0x_            
            float shard_damage_tick_interval; // 0x_            
            std::int32_t damage_tick_count; // 0x_            
            std::int32_t shard_damage_per_tick; // 0x_            
            std::int32_t m_nTotalBonusDamage; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Broodmother_InsatiableHunger because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Broodmother_InsatiableHunger) == 0x_);
    };
};
