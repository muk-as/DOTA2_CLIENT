...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            float blast_radius; // 0x1830            
            float blast_speed; // 0x1834            
            std::int32_t apply_parry_bonus; // 0x1838            
            float blind_duration; // 0x183c            
            float vuln_duration; // 0x1840            
            Vector m_vStartLoc; // 0x1844            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Kez_RavensVeil_Thinker because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Kez_RavensVeil_Thinker) == 0x1850);
    };
};
