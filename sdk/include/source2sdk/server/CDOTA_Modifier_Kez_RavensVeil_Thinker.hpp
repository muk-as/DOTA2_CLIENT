...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            float blast_radius; // 0x18b0            
            float blast_speed; // 0x18b4            
            std::int32_t apply_parry_bonus; // 0x18b8            
            float blind_duration; // 0x18bc            
            float vuln_duration; // 0x18c0            
            Vector m_vStartLoc; // 0x18c4            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Kez_RavensVeil_Thinker because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Kez_RavensVeil_Thinker) == 0x18d0);
    };
};
