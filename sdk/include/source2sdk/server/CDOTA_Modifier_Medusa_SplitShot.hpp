...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            std::int32_t damage_modifier; // 0x_            
            std::int32_t range; // 0x_            
            std::int32_t arrow_count; // 0x_            
            std::int32_t projectile_speed; // 0x_            
            source2sdk::client::ParticleIndex_t m_nSplitShotBowFXIndex; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::server::CDOTA_Modifier_Medusa_SplitShot) == 0x_);
    };
};
