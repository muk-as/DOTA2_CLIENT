...
namespace source2sdk::client
{
    ...
    {
    public:
        float sprout_heal_interval; // 0x1708        
        int32_t sprout_heal_per_second; // 0x170c        
        int32_t building_healing_pct; // 0x1710        
        [[maybe_unused]] std::uint8_t pad_0x1714[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Furion_Sprout_Heal because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Furion_Sprout_Heal) == 0x1718);
};
