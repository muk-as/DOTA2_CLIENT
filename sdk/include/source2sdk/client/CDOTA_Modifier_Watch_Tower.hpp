...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t m_iCapturingTeam; // 0x1708        
        float m_flCaptureProgress; // 0x170c        
        float m_flBonusTime; // 0x1710        
        bool m_bInitialSetupDone; // 0x1714        
        [[maybe_unused]] std::uint8_t pad_0x1715[0x3]; // 0x1715
        int32_t m_iOriginalTeam; // 0x1718        
        ...
        char m_hPreviousHero[0x4]; // 0x171c        
        ...
        char m_vecLastCreditedPlayerIDs[0x18]; // 0x1720        
        float m_flAccumulatedCaptureTime; // 0x1738        
        int32_t m_iBonusCount; // 0x173c        
        client::ParticleIndex_t m_nFxOutpostAmbient; // 0x1740        
        client::ParticleIndex_t m_nFxOutpostInitialAmbient; // 0x1744        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Watch_Tower because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Watch_Tower) == 0x1748);
};
