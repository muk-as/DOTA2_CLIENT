...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            float kill_heal_heroes; // 0x_            
            std::int32_t kill_creeps_penalty; // 0x_            
            float kill_heal_aoe; // 0x_            
            float kill_heal_duration; // 0x_            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Broodmother_Spiders_Milk because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Broodmother_Spiders_Milk) == 0x_);
    };
};
