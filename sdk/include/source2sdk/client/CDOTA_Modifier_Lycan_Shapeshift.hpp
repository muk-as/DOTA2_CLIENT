...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t bonus_night_vision; // 0x1708        
        float kill_duration_extension; // 0x170c        
        client::ParticleIndex_t m_nFXIndex; // 0x1710        
        int32_t m_iOriginalAttackCapability; // 0x1714        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Lycan_Shapeshift because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Lycan_Shapeshift) == 0x1718);
};
