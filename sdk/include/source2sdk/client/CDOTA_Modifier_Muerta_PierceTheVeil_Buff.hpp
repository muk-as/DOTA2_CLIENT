#pragma once
#include "source2sdk/client/CDOTA_Buff.hpp"
#include "source2sdk/client/ParticleIndex_t.hpp"
#include "source2sdk/source2gen.hpp"
#include <cstddef>
#include <cstdint>

// /////////////////////////////////////////////////////////////
// Module: client
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk::client
{
    // Registered alignment: unknown
    // Alignment: unknown
    // Standard-layout class: false
    // Size: 0x1728
    // Has VTable
    #pragma pack(push, 1)
    class CDOTA_Modifier_Muerta_PierceTheVeil_Buff : public client::CDOTA_Buff
    {
    public:
        int32_t modelscale; // 0x1708        
        int32_t bonus_damage; // 0x170c        
        int32_t spell_lifesteal; // 0x1710        
        int32_t attack_projectile_speed_bonus; // 0x1714        
        int32_t attack_range_bonus; // 0x1718        
        int32_t m_iOriginalAttackCapabilities; // 0x171c        
        client::ParticleIndex_t m_nScreenFXIndex; // 0x1720        
        [[maybe_unused]] std::uint8_t pad_0x1724[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Muerta_PierceTheVeil_Buff because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Muerta_PierceTheVeil_Buff) == 0x1728);
};
