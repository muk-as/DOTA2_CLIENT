...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            std::int32_t decay_damage; // 0x_            
            float radius; // 0x_            
            float decay_duration; // 0x_            
            std::int32_t str_steal; // 0x_            
            float creep_damage_multiplier; // 0x_            
            uint8_t _pad_[0x_];
        };
        #pragma pack(pop)
        ...
        static_assert(sizeof(source2sdk::client::C_DOTA_Ability_Undying_Decay) == 0x_);
    };
};
