...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t attack_speed_penalty; // 0x1708        
        int32_t attack_speed_from_movespeed; // 0x170c        
        int32_t slow_resistance; // 0x1710        
        [[maybe_unused]] std::uint8_t pad_0x1714[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Elder_Titan_Momentum because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Elder_Titan_Momentum) == 0x1718);
};
