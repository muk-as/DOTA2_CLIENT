...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t m_iKillerTeam; // 0x1858        
        int32_t m_iLastHealthPercent; // 0x185c        
        client::ParticleIndex_t m_nFXIndex; // 0x1860        
        [[maybe_unused]] std::uint8_t pad_0x1864[0x4]; // 0x1864
        ...
        char m_hAttackingHeroes[0x18]; // 0x1868        
        // metadata: MNetworkEnable
        bool m_bGoldenRoshan; // 0x1880        
        bool m_bIsNightTimeMode; // 0x1881        
        [[maybe_unused]] std::uint8_t pad_0x1882[0x6];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in C_DOTA_Unit_Roshan because it is not a standard-layout class
    static_assert(sizeof(C_DOTA_Unit_Roshan) == 0x1888);
};
