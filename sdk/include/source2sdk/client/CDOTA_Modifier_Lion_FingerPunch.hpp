#pragma once
#include "source2sdk/client/CDOTA_Buff.hpp"
#include "source2sdk/source2gen.hpp"
#include <cstddef>
#include <cstdint>

// /////////////////////////////////////////////////////////////
// Module: client
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk::client
{
    // Registered alignment: unknown
    // Alignment: unknown
    // Standard-layout class: false
    // Size: 0x1728
    // Has VTable
    #pragma pack(push, 1)
    class CDOTA_Modifier_Lion_FingerPunch : public client::CDOTA_Buff
    {
    public:
        int32_t punch_attack_range; // 0x1708        
        int32_t punch_bonus_damage_base; // 0x170c        
        int32_t punch_bonus_damage_per_stack; // 0x1710        
        int32_t cleave_starting_width; // 0x1714        
        int32_t cleave_ending_width; // 0x1718        
        int32_t cleave_distance; // 0x171c        
        int32_t cleave_damage; // 0x1720        
        int32_t m_iOriginalAttackCapabilities; // 0x1724        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Lion_FingerPunch because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Lion_FingerPunch) == 0x1728);
};
