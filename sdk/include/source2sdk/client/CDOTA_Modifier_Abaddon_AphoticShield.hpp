#pragma once
#include "source2sdk/client/CDOTA_Buff.hpp"
#include "source2sdk/entity2/GameTime_t.hpp"
#include "source2sdk/source2gen.hpp"
#include <cstddef>
#include <cstdint>

// /////////////////////////////////////////////////////////////
// Module: client
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk::client
{
    // Registered alignment: unknown
    // Alignment: unknown
    // Standard-layout class: false
    // Size: 0x1728
    // Has VTable
    #pragma pack(push, 1)
    class CDOTA_Modifier_Abaddon_AphoticShield : public client::CDOTA_Buff
    {
    public:
        float radius; // 0x1708        
        float damage_absorb; // 0x170c        
        int32_t regen; // 0x1710        
        int32_t m_nDamageAbsorbed; // 0x1714        
        float absorb_to_damage; // 0x1718        
        float absorb_damage_aoe; // 0x171c        
        entity2::GameTime_t m_flLastTickTime; // 0x1720        
        [[maybe_unused]] std::uint8_t pad_0x1724[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Abaddon_AphoticShield because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Abaddon_AphoticShield) == 0x1728);
};
