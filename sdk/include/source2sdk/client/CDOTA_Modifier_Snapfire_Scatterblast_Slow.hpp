...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            bool m_bIsPointBlank; // 0x17f8            
            uint8_t _pad17f9[0x3]; // 0x17f9
            std::int32_t movement_slow_pct; // 0x17fc            
            std::int32_t attack_slow_pct; // 0x1800            
            std::int32_t point_blank_dmg_bonus_pct; // 0x1804            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Snapfire_Scatterblast_Slow because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Snapfire_Scatterblast_Slow) == 0x1808);
    };
};
