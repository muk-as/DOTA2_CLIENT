#pragma once

#include "source2sdk/source2gen/source2gen.hpp"
#include <cstddef>
#include <cstdint>
#include "source2sdk/client/CDOTA_Buff.hpp"

// /////////////////////////////////////////////////////////////
// Module: client
// Created using source2gen - github.com/neverlosecc/source2gen
// /////////////////////////////////////////////////////////////

namespace source2sdk
{
    namespace client
    {
        // Registered alignment: unknown
        // Alignment: unknown
        // Standard-layout class: false
        // Size: 0x18a0
        // Has VTable
        #pragma pack(push, 1)
        class CDOTA_Modifier_Silencer_LastWord : public source2sdk::client::CDOTA_Buff
        {
        public:
            std::int32_t duration; // 0x1888            
            bool spellcast; // 0x188c            
            uint8_t _pad188d[0x3]; // 0x188d
            std::int32_t damage; // 0x1890            
            float debuff_duration; // 0x1894            
            float int_multiplier; // 0x1898            
            float radius; // 0x189c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CDOTA_Modifier_Silencer_LastWord because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::CDOTA_Modifier_Silencer_LastWord) == 0x18a0);
    };
};
