...
namespace source2sdk::client
{
    ...
    {
    public:
        int32_t strength_to_movement_pct; // 0x1708        
        int32_t move_speed_limit; // 0x170c        
        int32_t base_move_speed_penalty; // 0x1710        
        [[maybe_unused]] std::uint8_t pad_0x1714[0x4];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CDOTA_Modifier_Centaur_Horsepower because it is not a standard-layout class
    static_assert(sizeof(CDOTA_Modifier_Centaur_Horsepower) == 0x1718);
};
