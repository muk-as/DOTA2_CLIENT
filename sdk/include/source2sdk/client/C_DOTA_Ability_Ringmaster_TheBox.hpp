...
namespace source2sdk::client
{
    ...
    #pragma pack(push, 1)
    class C_DOTA_Ability_Ringmaster_TheBox : public client::C_DOTABaseAbility
    {
    public:
        bool m_bGrantedScepterCharges; // 0x600        
        [[maybe_unused]] std::uint8_t pad_0x601[0x7]; // 0x601
        ...
        // CUtlVector<CHandle<client::C_BaseEntity>> m_vecBoxedUnits;
        char m_vecBoxedUnits[0x18]; // 0x608        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in C_DOTA_Ability_Ringmaster_TheBox because it is not a standard-layout class
    static_assert(sizeof(C_DOTA_Ability_Ringmaster_TheBox) == 0x620);
};
